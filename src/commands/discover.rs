...
pub fn discover(args: &DiscoverArgs) -> HandledResult<()> {
    ...
    for hostname in hostnames {
        let (host, mut resources, mut resource_groups) =
            discover_one_host(&hostname, args.verbose)?;
        config.hosts.push(host);
        config.resources.append(&mut resources);
        config.resource_groups.append(&mut resource_groups);
    }
...
fn discover_one_host(
    ...
    let resource_groups = zpool_resources
        .iter()
        .map(|r| config::ResourceGroup {
            root: r.name.to_owned(),
            home_host: hostname.to_owned(),
            failover_hosts: vec![],
        })
        .collect();
